...
# class UserPanelDashboardAPIView(APIView):
#     def get(self, request):
#         contacts = Contact.objects.filter(user=request.user)  # Filter contacts by the authenticated user
#         serializer = ContactSerializer(contacts, many=True)
#         return Response(serializer.data)
...
#             return redirect('login_page')
#         else:
#             # Retrieve all contacts from the database, regardless of the user
#             contacts = Contact.objects.filter(user=request.user).order_by('name')
#             # Render the user panel dashboard page with the contacts as context
#             self.extra_context = {'contacts': contacts}
#             return super().get(request)
#             # 2 other ways for rendering manually
#             # return render(request, self.template_name, {'contacts': contacts})
#             # return render(request, 'user_panel_module/user_panel_dashboard_page.html', {'contacts': contacts})
#     @classmethod
#     def user_panel_menu_component(cls, request):
#         if not request.user.is_authenticated:
#             return redirect('login_page')
...
#             # Retrieve all contacts from the database, regardless of the user
#             contacts = Contact.objects.order_by('name')
#             # Render the contact list component with the contacts as context
#             return render(request, 'contacts/contact_list.html', {'contacts': contacts})
...
#         # Example: Create a new contact from the form data
#         name = request.POST.get('name')
#         phone_number = request.POST.get('phone_number')
#         email = request.POST.get('email')
...
#         else:
#             # Return a validation error response
#             return Response(contact_form.errors, status=status.HTTP_400_BAD_REQUEST)
...
#     # def get(self, request):
#     #     # Return a response with any necessary data for the GET request
#     #     return Response({'message': 'This is the GET request'}, status=status.HTTP_200_OK)
#     # def post(self, request):
#     #     # Handle the form submission when adding a new contact
#     #     contact_form = ContactForm(request.data)
#     #     if contact_form.is_valid():
...
#     #             user = request.user
#     #             # Create a new contact object
#     #             Contact.objects.create(name=name, phone_number=phone_number, email=email, user=user)
